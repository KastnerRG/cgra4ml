...
#
# Fan-out 1: A feeds B0 and B1 (input fan-out, same as chained_matmul.py)
# Fan-out 2: P feeds B3 and B4 (intermediate fan-out — the new capability under test)
...
class UserModel(XModel):
    def __init__(self, sys_bits, x_int_bits, *args, **kwargs):
        ...
        self.b_p  = XBundle(core=XDense(k_int_bits=0, b_int_bits=0, units=N, use_bias=False, act=relu()))
    ...
    def call(self, a):
        ...
        z1 = self.b_ab(a)               # Bundle 0: Z1 = A @ B
        z2 = self.b_ac(a)               # Bundle 1: Z2 = A @ C  (fan-out on a)
        p  = self.b_p (z1, w_src=z2)    # Bundle 2: P  = Z1 @ Z2
        z3 = self.b_pd(p)               # Bundle 3: Z3 = P @ D  (fan-out on p)
        z4 = self.b_pe(p)               # Bundle 4: Z4 = P @ E  (fan-out on p)
