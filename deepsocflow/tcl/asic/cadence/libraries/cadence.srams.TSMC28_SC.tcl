...
set tech_files(SRAM_WEIGHTS_LVT_BC_LIB) "$paths(SRAM_LIB_WEIGHTS_Paths)/sram_weights_ffg_cbestt_0p77v_0p88v_m40c.lib"
    set tech_files(ALL_BC_LIBS) $tech_files(SRAM_WEIGHTS_LVT_BC_LIB)
set tech_files(SRAM_WEIGHTS_LVT_WC_LIB) "$paths(SRAM_LIB_WEIGHTS_Paths)/sram_weights_ssg_cworstt_0p99v_0p72v_125c.lib"
    set tech_files(ALL_WC_LIBS) $tech_files(SRAM_WEIGHTS_LVT_WC_LIB)
set tech_files(SRAM_WEIGHTS_LVT_TC_LIB) "$paths(SRAM_LIB_WEIGHTS_Paths)/sram_weights_tt_ctypical_0p99v_0p70v_85c.lib"
    set tech_files(ALL_TC_LIBS) $tech_files(SRAM_WEIGHTS_LVT_TC_LIB)

set tech_files(SRAM_WEIGHTS_RVT_BC_LIB) "$paths(SRAM_LIB_WEIGHTS_Paths)/sram_weights_ffg_cbestt_0p77v_0p88v_m40c.lib"
    lappend tech_files(ALL_BC_LIBS) $tech_files(SRAM_WEIGHTS_RVT_BC_LIB)
set tech_files(SRAM_WEIGHTS_RVT_WC_LIB) "$paths(SRAM_LIB_WEIGHTS_Paths)/sram_weights_ssg_cworstt_0p77v_0p81v_125c.lib"
    lappend tech_files(ALL_WC_LIBS) $tech_files(SRAM_WEIGHTS_RVT_WC_LIB)
set tech_files(SRAM_WEIGHTS_RVT_TC_LIB) "$paths(SRAM_LIB_WEIGHTS_Paths)/sram_weights_tt_ctypical_0p99v_0p90v_85c.lib"
    lappend tech_files(ALL_TC_LIBS) $tech_files(SRAM_WEIGHTS_RVT_TC_LIB)

set tech_files(SRAM_WEIGHTS_HVT_BC_LIB) "$paths(SRAM_LIB_WEIGHTS_Paths)/sram_weights_ffg_cbestt_0p77v_1p05v_m40c.lib"
    lappend tech_files(ALL_BC_LIBS) $tech_files(SRAM_WEIGHTS_HVT_BC_LIB)
set tech_files(SRAM_WEIGHTS_HVT_WC_LIB) "$paths(SRAM_LIB_WEIGHTS_Paths)/sram_weights_ssg_cworstt_0p77v_0p90v_125c.lib"
    lappend tech_files(ALL_WC_LIBS) $tech_files(SRAM_WEIGHTS_HVT_WC_LIB)
set tech_files(SRAM_WEIGHTS_HVT_TC_LIB) "$paths(SRAM_LIB_WEIGHTS_Paths)/sram_weights_tt_ctypical_0p99v_1p05v_85c.lib"
    lappend tech_files(ALL_TC_LIBS) $tech_files(SRAM_WEIGHTS_HVT_TC_LIB)

# SRAM Libs - SRAM Edges

set tech_files(STANDARD_CELLS_LVT_BC_LIB) "$paths(SRAM_LIB_EDGES_Paths)/sram_weights_ffg_cbestt_0p77v_0p88v_m40c.lib"
    set tech_files(ALL_BC_LIBS) $tech_files(STANDARD_CELLS_LVT_BC_LIB)
set tech_files(STANDARD_CELLS_LVT_WC_LIB) "$paths(SRAM_LIB_EDGES_Paths)/sram_weights_ssg_cworstt_0p99v_0p72v_125c.lib"
    set tech_files(ALL_WC_LIBS) $tech_files(STANDARD_CELLS_LVT_WC_LIB)
set tech_files(STANDARD_CELLS_LVT_TC_LIB) "$paths(SRAM_LIB_EDGES_Paths)/sram_weights_tt_ctypical_0p99v_0p70v_85c.lib"
    set tech_files(ALL_TC_LIBS) $tech_files(STANDARD_CELLS_LVT_TC_LIB)

set tech_files(STANDARD_CELLS_RVT_BC_LIB) "$paths(SRAM_LIB_EDGES_Paths)/sram_weights_ffg_cbestt_0p77v_0p88v_m40c.lib"
    lappend tech_files(ALL_BC_LIBS) $tech_files(STANDARD_CELLS_RVT_BC_LIB)
set tech_files(STANDARD_CELLS_RVT_WC_LIB) "$paths(SRAM_LIB_EDGES_Paths)/sram_weights_ssg_cworstt_0p77v_0p81v_125c.lib"
    lappend tech_files(ALL_WC_LIBS) $tech_files(STANDARD_CELLS_RVT_WC_LIB)
set tech_files(STANDARD_CELLS_RVT_TC_LIB) "$paths(SRAM_LIB_EDGES_Paths)/sram_weights_tt_ctypical_0p99v_0p90v_85c.lib"
    lappend tech_files(ALL_TC_LIBS) $tech_files(STANDARD_CELLS_RVT_TC_LIB)

set tech_files(STANDARD_CELLS_HVT_BC_LIB) "$paths(SRAM_LIB_EDGES_Paths)/sram_weights_ffg_cbestt_0p77v_1p05v_m40c.lib"
    lappend tech_files(ALL_BC_LIBS) $tech_files(STANDARD_CELLS_HVT_BC_LIB)
set tech_files(STANDARD_CELLS_HVT_WC_LIB) "$paths(SRAM_LIB_EDGES_Paths)/sram_weights_ssg_cworstt_0p77v_0p90v_125c.lib"
    lappend tech_files(ALL_WC_LIBS) $tech_files(STANDARD_CELLS_HVT_WC_LIB)
set tech_files(STANDARD_CELLS_HVT_TC_LIB) "$paths(SRAM_LIB_EDGES_Paths)/sram_weights_tt_ctypical_0p99v_1p05v_85c.lib"
