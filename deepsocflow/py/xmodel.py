...
def verify_inference(model, hw, SIM, SIM_PATH='', TRACE=False, SIM_TYPE='fpga', SRAM_GEN=False, RUN=1, RUNTYPE='rtl', SRAM_COMPILERS=None):

    '''
    RUN SIMULATION
    '''
    hw.simulate(SIM=SIM, SIM_PATH=SIM_PATH, TRACE=TRACE, SIM_TYPE=SIM_TYPE, SRAM_GEN=SRAM_GEN, RUN=RUN, RUNTYPE=RUNTYPE, SRAM_COMPILERS=SRAM_COMPILERS)
