...
def export_inference(model, hw, batch_size=1):
    ...
    with open (f'./config_fw.h', 'w') as ch:
        ...
        for ib, b in enumerate(BUNDLES):
            ...
            ch.write(     f".is_bias={1*(b.core.b is not None):<3}, .is_flatten={1*(b.flatten is not None):<3}, .is_softmax={1*(b.softmax is not None):<3}, .transpose_w_src={1 if (b.transpose_w_src and b.w_src_ib is not None) else 0:<3}, ")
