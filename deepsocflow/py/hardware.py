...
''')
    ...
    def simulate(self, SIM='verilator', SIM_PATH='', TRACE=False):
        ...
        ################################################
        if SIM == 'vcs':
            cmd_c_comp = ["gcc", "-std=c99", "-shared", "-fPIC", "-DSIM"] + ["-I" + self.TB_C_HEADER[0]] + ["-I" + "../"] +["-o", "dpi_compiled.so"] + self.TB_C_SIM
            ...
            cmd_vcs = [ "vcs", "-sverilog", "-full64", "-kdb", "-debug_access+all", "-timescale=1ns/1ps"] + ["+define+ARM_UD_MODEL+INITIALIZE_MEMORY"] + ["+incdir+" + self.INC_DIR_DEFINE[0] + "+" + self.INC_DIR_CONFIG + "+" + self.INC_DIR_SRAMS] + ["-top", self.TB_MODULE] + self.SOURCES_SRAM + ["-l", "VCS_Comp.log"] #RTL Simulation with Asic SRAMs
