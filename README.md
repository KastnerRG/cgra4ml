...
3.2.a. Run bare-metal on a ZYNQ FPGA:
...
**Prerequisites: Linux SD Card Image**

Download the AMD Linux SD card image for ZCU104 from [AMD Xilinx's downloads page](https://www.xilinx.com/support/download/index.html/content/xilinx/en/downloadNav/embedded-design-tools.html) (look for pre-built images for ZCU104). Flash it to an SD card.
...
**Generate the Firmware Bundle**

Run one of the following commands. This will generate the firmware bundle in `cgra4ml-fw`.

```bash
# Runs make vivado first
make hw
...
**Build the CGRA4ML Driver and Inference App**

```bash
# Clone kernel source and prepare build tree (required once)
make kernel_prepare

# Build kernel module + userspace test apps
...
```bash
# Kernel driver
scp linux_driver/cgra4ml_drv.ko <user>@<board-ip>:/tmp/

# Test applications
scp linux_test/inference linux_test/reg_test \
    linux_test/ioctl_test linux_test/dma_buf_test \
    <user>@<board-ip>:/tmp/

# Model data (generated by `python run/example.py`)
scp run/work/vectors/wbx.bin <user>@<board-ip>:/tmp/

# Firmware bunle
scp run/work/cgra4ml-fw <user>@<board-ip>:/tmp/
```
...
```bash
# 1. Load the firmware
sudo mkdir -p /lib/firmware/xilinx/cgra4ml
sudo cp cgra4ml-fw/* /lib/firmware/xilinx/cgra4ml/
sudo dfx-mgr-client -load cgra4ml

# 2. Load the kernel driver
sudo insmod /tmp/cgra4ml_drv.ko
...
See `linux_driver/linux_driver_README.md` and `linux_test/linux_test_README.md` for detailed documentation.
