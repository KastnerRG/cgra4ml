...
class FPGA_Reshaper():
    def __init__(self, 
        ...
        self.num_conv_layers = len([c for c in self.model_dict.keys() if ('conv' in c)])
    ...
    def make_weights(self, i):
        ...
        kernel = self.model_dict[f'{self.conv_prefix}_{i}'].weights     # (3, 3, cin, cout)        : (3, 3, 64, 128) - Layer 3
        biases = self.model_dict[f'{self.conv_prefix}_{i}'].biases      # (cout,)                  : (128,)          - Layer 3
        ...
        if k == 3:
            ...
            bias = self.model_dict[f'{self.conv_prefix}_{i}'].biases  # (cout,)         : (128,) - Layer 3
            ...
            biases = self.model_dict[f'{self.conv_prefix}_{i}'].biases                      # (cout,)                  : (64,)
        ...
        for i in range(1,self.num_conv_layers+1):
            ...
            if self.all_bin:
                if self.weights_all is None:
                    ...
                    self.weights_all = np.append(self.weights_all,current_weights)
